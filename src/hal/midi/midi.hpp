#ifndef HAL_MIDI_DEFINED
#define HAL_MIDI_DEFINED
namespace midi {
		...
		explicit midi1_packet(message_t const& msg) {
			visit(visitor{
				[&](noteOnMessage const& msg) {
					status = (uint8_t)(0x90 | msg.channel), lo = msg.note, hi = msg.velocity;
				},
				[&](noteOffMessage const& msg) {
					status = (uint8_t)(0x80 | msg.channel), lo = msg.note, hi = msg.velocity;
				},
				[&](programChangeMessage const& msg) {
					status = (uint8_t)(0xC0 | msg.channel), lo = msg.program, hi = 0;
				},
				[&](pitchBendMessage const& msg) {
					status = (uint8_t)(0xE0 | msg.channel), lo = msg.level & 0x7F, hi = msg.level >> 7;
				},
				[&](controlChangeMessage const& msg) {
					status = (uint8_t)(0xB0 | msg.channel), lo = msg.controller, hi = msg.value;
				},
			}, msg);
		}
